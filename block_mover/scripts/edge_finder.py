...
for i in range(1, 11):
    ...
    """
    contours = cv2.findContours(canny_img)
    ...
    #canny_img = cv2.erode(canny_img, np.ones((4,4), np.uint8), iterations=1)
    ...
    # Standard Hough
    """
    lines = cv2.HoughLines(canny_img,1,np.pi/360,10)
    ...
    minLineLength = 40
    maxLineGap = 5
    lines = cv2.HoughLinesP(canny_img,1,np.pi/180,20,maxLineGap,minLineLength)
    ...
    harris_img[dst>0.01*dst.max()]=0

    brisk = cv2.BRISK()
    kp_brisk = brisk.detect(img)
    brisk_img = img.copy()
