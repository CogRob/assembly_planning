...
class Agent(object):
    ...
    def __init__(self, limb, hover_distance_=0.0, verbose=True):
        ...
        rospy.loginfo("Setting-up control variables")
        ...
        # Baxter system setup
        rospy.loginfo("Setting up control pipeline")
        ...
        self._start_angles = {
            self._limb_name + '_e0': -0.20632041597058814,
            self._limb_name + '_e1': 1.5412671966278695,
            self._limb_name + '_s0': 0.35204859081970247,
            self._limb_name + '_s1': -0.9376457565949592,
            self._limb_name + '_w0': 0.16643691548556738,
            self._limb_name + '_w1': 0.941864203761644,
            self._limb_name + '_w2': -0.11926700625809092
        }
    ...
    def _transport(self, pose=None):
        """transports the end-effector to the given position"""
        overhead_pose = Pose()

        rospy.loginfo("Pose to go to: %s", str(pose))

        overhead_pose.position = Point(
            x=pose.x, y=pose.y, z=self._hover_distance)
    ...
    def _detect(self):  # , orientation):
        ...
        for block_obs in block_obs_msg.ws_obs:
            ...
            self._env_state.add_block(new_block)
    ...
    def add_block_to_ws(self, block):
        self._env_state.add_block(block)
    ...
    def executor(self, action, constraints=Constraint()):
        """
        Interfaces with planner and perception

        Receives actions + perceptions objects and actuates Baxter
        """
